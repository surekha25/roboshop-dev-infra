...
resource "aws_lb_target_group" "catalogue" {
  ...
  protocol = "HTTP"
  vpc_id   = local.vpc_id
  deregistration_delay = 60 # waiting period before auto deleting the instances when no traffic
...
resource "aws_launch_template" "catalogue" {
  ...
  # when we run terrafrom apply again, a new version is created with new AMI ID
